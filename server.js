...
// GET One
app.get("/creature/:id", async (req, res) => {
  const { id } = req.params;

  try {
    const result = await pool.query(
      "SELECT * FROM creatures WHERE creature_id = $1",
      [id]
    );
    res.json(result.rows[0]).status(200);
  } catch (err) {
...
app.post("/creature", async (req, res) => {
  const { creature_lvl, creature_name, creature_plane, creature_url } =
    req.body;
...
//DELETE one
app.delete("/creature/:id", async (req, res) => {
  const { id } = req.params;
...
//UPDATE one
app.put("/creature/:id", async (req, res) => {
  const { id } = req.params;
  let input = "";
  ...
  const SQLcode = `UPDATE creatures SET ${input} WHERE creature_id = ${id} RETURNING *;`;
  try {
    const result = await pool.query(SQLcode);
    res.status(200).send(result.rows[0]);
  } catch (err) {
